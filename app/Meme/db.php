<?php
/**
 * Meme Maker DB Class. Derived from WP DB Class
 *
...
/**
 * Meme Maker Database Access Abstraction Object
 *
 ...
 * available.
 *
 * @link http://codex.Meme Maker.org/Function_Reference/wpdb_Class
 *
 * @package Meme Maker
 * @subpackage Database
 * @since 0.71
...
class wpdb {
	...
	/**
	 * Meme Maker table prefix
	 *
	 * You can set this to have multiple Meme Maker installations
	...
	/**
	 * List of Meme Maker per-blog tables
	 *
	 * @since 2.5.0
	 * @access private
	 * @see wpdb::tables()
	...
	/**
	 * List of deprecated Meme Maker tables
	 *
	...
	/**
	 * List of Meme Maker global tables
	 *
	 * @since 3.0.0
	 * @access private
	 * @see wpdb::tables()
	...
	/**
	 * Meme Maker Comment Metadata table
	 *
	 * @since 2.9.0
	 * @access public
	 * @var string
	...
	/**
	 * Meme Maker Post Metadata table
	 *
	 * @since 1.5.0
	 * @access public
	 * @var string
	...
	/**
	 * Meme Maker Term Relationships table
	 *
	 * @since 2.3.0
	 * @access public
	 * @var string
	...
	/**
	 * Meme Maker Term Taxonomy table
	 *
	 * @since 2.3.0
	 * @access public
	 * @var string
	...
	/**
	 * Meme Maker User Metadata table
	 *
	 * @since 2.3.0
	 * @access public
	 * @var string
	 ...
	 * to the database.
	 *
	 * @link http://core.trac.Meme Maker.org/ticket/3354
	 * @since 2.0.8
	 *
	 * @param string $dbuser MySQL database user
	 * @param string $dbpassword MySQL database password
	...
	/**
	 * Sets the table prefix for the Meme Maker tables.
	 *
	 * @since 2.5.0
	 *
	...
	/**
	 * Returns an array of Meme Maker tables.
	 *
	 * Also allows for the CUSTOM_USER_TABLE and CUSTOM_USER_META_TABLE to
	 * override the Meme Maker users and usersmeta tables that would otherwise
	...
	function select( $db, $dbh = null) {
		...
		if ( !@mysql_select_db( $db, $dbh ) ) {
			$this->ready = false;
			$this->bail( sprintf( 'Cannot connect to DB. Please check your settings.', $db, $this->dbuser ), 'db_select_fail' );
			return;
		}
	}
	...
	function print_error( $str = '' ) {
		...
		if ( $caller = $this->get_caller() )
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR_FULL*/'Meme Maker database error %1$s for query %2$s made by %3$s'/*/WP_I18N_DB_QUERY_ERROR_FULL*/, $str, $this->last_query, $caller );
		else
			$error_str = sprintf( /*WP_I18N_DB_QUERY_ERROR*/'Meme Maker database error %1$s for query %2$s'/*/WP_I18N_DB_QUERY_ERROR*/, $str, $this->last_query );
		...
		if ( is_multisite() ) {
			$msg = "Meme Maker database error: [$str]\n{$this->last_query}\n";
			...
			print "<div id='error'>
			<p class='wpdberror'><strong>Meme Maker database error:</strong> [$str]<br />
			<code>$query</code></p>
			</div>";
		}
	}
	...
	function db_connect() {
		...
		if ( !$this->dbh ) {
			$this->bail( sprintf( 'Cannot Connect to Database. Check your settings.', $this->dbhost ), 'db_connect_fail' );
	...
	function check_database_version() {
		...
		if ( version_compare($this->db_version(), $required_mysql_version, '<') )
			return new WP_Error('database_version', sprintf( __( '<strong>ERROR</strong>: Meme Maker %1$s requires MySQL %2$s or higher' ), $wp_version, $required_mysql_version ));
	}

	/**
	 * Whether the database supports collation.
	 *
	 * Called when Meme Maker is generating the table scheme.
	 *
	 * @since 2.5.0
	 *
